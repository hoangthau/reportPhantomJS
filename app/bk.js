function CreatePieChart (scope, element, attrs) {
  var defaultConfig={
    properties: {
      width:'100%',
      height:'100%',
  ...
  var name = obj.chartConfig.mapping.name;
  var value = obj.chartConfig.mapping.value;
  var label = obj.chartConfig.label;
  var innerLabel= obj.chartConfig.label.inner || defaultConfig.label.inner;
  var outerLabel = obj.chartConfig.label.outer || defaultConfig.label.outer;
  var legend = obj.chartConfig.legend || defaultConfig.legend;
  var tooltip = obj.chartConfig.tooltip || defaultConfig.tooltip;
  var svgWith=obj.chartConfig.properties.width;
  var svgHeight=obj.chartConfig.properties.height;
  var id=obj.chartConfig.properties.id || defaultConfig.properties.id;
  var legendCenterDonut = obj.chartConfig.legend.showCenterDonut || false;
  //disable outsite label
  var pieDistance=30;

  // var radius = obj.chartConfig.properties.outerRadius;
  var innerRadius=obj.chartConfig.properties.innerRadius;
  var  marginTop = obj.chartConfig.properties.marginTop,
    marginLeft = obj.chartConfig.properties.marginLeft;
  // colors = d3.scale.category20();

  // Color Scale Handling...
  var colorScale = d3.scale.category20c();
  function setColors(){
    var colors = obj.chartConfig.properties.colors;
    switch (colors) {
    case 'colorScale10':
    ...
    if(getTotal(currData)>0){
      ...
      var onMouseOver=function(d){
        ...
        if (this._listenToEvents) {
          ...
          if(obj.chartConfig.properties.hoverColor){
            d3.select(this).style('fill', obj.chartConfig.properties.hoverColor);
          }else{
      ...
      var drawChildren=function(i){
        ...
        if (obj.onClick) {
          obj.onClick(currData[i],currData);
        }
      };

      obj.chartConfig.api.setActive=function(i){
        setActive(i);
      };
      obj.chartConfig.api.resetAllActive=function(){
        svg.selectAll('path').attr('transform', 'translate(0,0)');
      };
      obj.chartConfig.api.resetActive=function(i){
        resetActive(i);
      };
      // DRAW INSIDE LABELS
...
//              });
    ...
    obj.chartConfig.api.drawChildren=function(i){
      drawChildren(i);
    };
  }
  obj.chartConfig.api.updateChart=function(newData){
    updateGraph(newData);
  };
  // Start by updating graph at root level
  // obj.dataAndColors=[obj.chartData,obj.chartConfig.properties.colors];

  //obj.$watch('chartData', function (newVal) {
  //  if (newVal) {
  updateGraph(obj.chartData);
  //  }
  //});
  //obj.$on('chart.resize', function(){
...
//table
var tableHeader = '', i;
if (config.table && config.table.hasOwnProperty('header')) {
  for (i = 0; i < config.table.header.length; i++) {
    tableHeader += '<td>' + config.table.header[i] + '</td>';
  }
}
var tableContent = '';
if (config.table && config.table.hasOwnProperty('row')) {
  for (i = 0; i < config.table.row.length; i++) {
    var td = '';
    for (var itemRow in config.table.row[i]) {
      td += '<td>' + config.table.row[i][itemRow] + '</td>';
    }
...
function scaleAndClipPage(bodyObj) {
  var zoom = 1,
    pageWidth = bodyObj.width,
    pageHeight = bodyObj.height;
  ...
  if (outputFormat === 'pdf') {
    ...
    page.paperSize = {width: pageWidth + 2, height: pageHeight + 2};
  }
}

scaleAndClipPage(bodyObj);

var filterRows = rows.filter(function(item){
  var cond = true;
  for(i = 0; i < defaultOpts.filters.length; i++){
    var f = item[defaultOpts.filters[i].id] === defaultOpts.filters[i].value;
    if(!f){
      cond = false;
      break;
    }
